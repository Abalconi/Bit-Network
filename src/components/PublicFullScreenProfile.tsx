...
export const PublicFullScreenProfile: React.FC<PublicFullScreenProfileProps> = ({
  ...
  return (
    <div className="min-h-screen w-full bg-white text-slate-900 flex flex-col justify-between selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* Botón flotante para acceder a editar datos desde el CRM (SOLO para el dueño autenticado) */}
      {isAuthenticated && onOpenCrm && (
        <button
          onClick={onOpenCrm}
      ...
      <div className="relative w-full h-64 sm:h-80 md:h-96 lg:h-[420px] overflow-hidden bg-slate-900 group">
        ...
        {/* Botón directo para subir/cambiar foto de fondo (SOLO para el dueño autenticado) */}
        {isAuthenticated && onUpdateUser && (
          <button
            type="button"
      ...
      <div className="relative -mt-20 sm:-mt-28 md:-mt-32 flex flex-col items-center px-4 sm:px-6 w-full max-w-5xl mx-auto flex-1 pb-16">
        ...
        <div className="relative group/avatar">
          ...
          {/* Botón directo para subir/cambiar foto de perfil (SOLO para el dueño autenticado) */}
          {isAuthenticated && onUpdateUser && (
            <button
              type="button"
        ...
        {/* ============================================================== */}
        {/* 6. LEMA INFERIOR & ACCESO SEGURO PROPIETARIO                   */}
        {/* ============================================================== */}
        <div className="mt-12 flex flex-col items-center text-center">
          ...
          {/* Enlace discreto de acceso con credenciales para el dueño */}
          <div className="mt-8 pt-4 border-t border-slate-100 w-full max-w-xs flex justify-center items-center">
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                {onOpenCrm && (
                  <button
                    onClick={onOpenCrm}
                    className="inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition cursor-pointer"
                  >
                ...
                {onLogout && (
                  <button
                    onClick={onLogout}
                    className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 hover:text-rose-600 transition cursor-pointer ml-2"
                  >
              ...
              onRequestLogin && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 text-[11px] font-medium text-slate-400 hover:text-slate-700 transition cursor-pointer py-1 px-2.5 rounded-full hover:bg-slate-50"
                  title="Acceso administrativo con contraseña"
                >
